...
The dev container bind-mounts the repository into `/app` and stores Linux dependencies in the named volume `metapi_node_modules`. It installs production dependencies, adds only the required dev server tools with `--omit=dev`, then rebuilds `esbuild`, `sharp`, and `better-sqlite3` inside Linux to avoid Electron desktop packaging dependencies. The install step is skipped on later starts when those binaries already exist. If `package-lock.json` changes, recreate the dev container or remove the named volume.
